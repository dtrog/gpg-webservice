...
def get_admin_gpg_keys() -> dict:
    ...
    try:
        ...
        logger.error("Failed to parse ADMIN_GPG_KEYS: %s", e)
        return {}
...
def verify_admin_token(token: str) -> Optional[str]:
    ...
    try:
        ...
        if time.time() - token_time > ADMIN_TOKEN_VALIDITY_SECONDS:
            logger.warning("Expired admin token for %s", username)
            return None
        ...
        if not hmac.compare_digest(signature, expected_sig):
            logger.warning("Invalid HMAC for admin token: %s", username)
            return None

        # Verify username is in admin list
        admin_keys = get_admin_gpg_keys()
        if username not in admin_keys:
            logger.warning("Token for non-admin user: %s", username)
            return None

        return username

    except (ValueError, IndexError) as e:
        logger.warning("Malformed admin token: %s", e)
        return None
...
def get_challenge():
    ...
    admin_keys = get_admin_gpg_keys()
    if username not in admin_keys:
        logger.warning("Challenge requested for non-admin: %s", username)
    ...
    logger.info("Generated auth challenge for %s", username)
...
def verify_challenge():
    ...
    if time.time() > challenge_data['expires_at']:
        ...
        logger.warning("Expired challenge for %s", username)
    ...
    if not is_valid:
        logger.warning(
            "Failed GPG signature verification for %s: %s", username, error
        )
        return jsonify({
    ...
    logger.info("Issued admin token for %s", username)
