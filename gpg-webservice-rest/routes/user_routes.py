...
def register():
    ...
    This allows AI agents to regenerate their password
    from their immutable contract.
...
def get_session_key():
    ...
    return jsonify({
        ...
        'window_start': session_info.window_start,
        'expires_at': session_info.expires_at,
        'note': 'This session key is deterministically derived. You can get '
                'the same key by calling /login again within the same hour.'
    }), 200
...
def get_api_key():
    ...
    return jsonify({
        ...
        'api_key': session_info.api_key,
        'expires_at': session_info.expires_at,
        'note': 'API keys are now deterministic session keys that expire '
                'hourly. Use /login or /get_session_key to refresh.'
    }), 200
...
def regenerate_api_key():
    ...
    return jsonify({
        ...
        'session_window': session_info.window_index,
        'expires_at': session_info.expires_at,
        'note': 'With deterministic keys, regeneration is automatic. '
                'Simply call /login when your key expires to get a fresh one '
...
def upload_keys():
    ...
    # Verify password
    user_service = UserService()
    error = user_service.login_user(user.username, password)
    if error:
    ...
    if 'public_key_file' in request.files:
        ...
        if public_key_file.filename:
            ...
            if public_key:
                ...
                public_key = PgpKey(user_id=user.id,
    ...
    if 'private_key_file' in request.files:
        ...
        if private_key_file.filename:
            ...
            private_key = PgpKey.query.filter_by(
                user_id=user.id, key_type=PgpKeyType.PRIVATE
            ).first()
            if private_key:
    ...
    audit_logger.log_event(
        ...
        user_id=user.id,
        username=user.username,
        message=f'Keys uploaded for {user.username}: '
                f'{", ".join(updated_keys)}'
    )
