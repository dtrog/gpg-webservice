...
def get_api_key():
    ...
    # Import get_user_by_api_key to retrieve the API key hash
    from services.auth_service import AuthService
    from db.database import db
    from models.user import User

    # Get the raw API key from the database (it's hashed)
    # We need to return a masked version for security
    ...
    return jsonify({
        ...
        'note': 'For security reasons, the full API key cannot be retrieved. It was only shown once during registration. If lost, you must contact support or re-register.'
    }), 200
